    ...
    return (
      <div className="common-page-title" style={_style}>
        <div className="common-page-title-content">
          ...
          <div className="common-page-title-con coupon-top">
            {_title &&
              _title.length &&
              _title.map((item) => {
                return <span key={"coupon-tab-"+item.tab} className={tabIndex==item.tab?"coupon-active":""} onClick={()=>{
                    this.props.changeTab(item.tab)
                }}>{item.title}</span>;
              })}
          </div>
